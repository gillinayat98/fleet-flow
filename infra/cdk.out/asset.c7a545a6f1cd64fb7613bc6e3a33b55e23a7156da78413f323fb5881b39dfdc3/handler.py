...
def _generate_sql(question: str, schema: str) -> str:
    system = (
        ...
        "- Give every table an alias and qualify EVERY column with its alias "
        "(e.g. fr.unit_no) — several tables share column names like unit_no.\n"
...
def _answer(payload: dict) -> dict:
    ...
    # The model can produce valid-looking SQL that fails at execution
    # (e.g. an ambiguous column). Degrade gracefully — surface the SQL and the
    # engine's message rather than crashing.
    try:
        header, data = _run_athena(sql)
    except RuntimeError as e:
        return {"question": question, "sql": sql,
                "error": f"the generated query failed to run: {e}"}
...
def handler(event, context):
    ...
    if isinstance(event, dict) and "requestContext" in event:
        try:
            ...
            payload = {}
        # Catch everything so the browser always gets a CORS-headed JSON
        # response (a bare Lambda crash returns a 502 with no CORS headers,
        # which the browser reports only as an opaque "Failed to fetch").
        try:
        ...
        # Always 200: a blocked or failed-to-run query is a valid assistant
        # response ("I couldn't answer that"), carried in the body's "error"
        # field for the UI to display — not an HTTP-level failure.
        return {
            "statusCode": 200,
